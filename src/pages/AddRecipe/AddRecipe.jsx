...
const AddRecipe = () => {
  ...
  useAutoResizeTextarea(styles.recipeTextArea);

  return (
    <div className={styles.recipeContainer}>
      <div className={styles.titleAndNavWrapper}>
        <div className={styles.navCrumbs}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className={styles.formLayout}>
        <div className={styles.formContainer}>
          ...
          <div>
            <div className={styles.recipeNameInputWrapper}>
              <Input
                type="text"
                name="title"
                register={register}
              ...
              />
              {errors.title && <p className={styles.errorMessageTitle}>{errors.title.message}</p>}
            </div>

            <div className={styles.recipeDetails}>
              <div className={styles.categoryAndTimeSection}>
                <div className={styles.recipeDetails}>
                  <IngredientSelector
                    control={control}
                    register={register}
                  ...
                  {errors.selectedIngredients && (
                    <p className={styles.errorMessage}>{errors.selectedIngredients.message}</p>
                  )}
                </div>
              </div>
            </div>
            <div className={styles.recipeSteps}>
              <h2 className={styles.sectionHeader}>Recipe preparation</h2>
              <div
                className={`${styles.textAreaContainer} ${stylesInput.form__group} ${stylesInput.field}`}
              >
                <textarea
                  ...
                  maxLength="none"
                  className={`${styles.recipeTextArea} ${stylesInput.form__field}`}
                />
                <label className={`${styles.labelPreparation} ${stylesInput.form__label}`}>
                  Enter recipe
                </label>
            ...
            <div className={styles.actionButtons}>
              <IconButton
                ...
                styleSVG={styles.deleteIcon}
              />

              <Button text="Publish" type="submit" classname={styles.submitButton} />
            </div>
          </div>
        </div>
      </form>
